...
func TestTextChannelID_Parse(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		m, s := state.NewMocker(t)
		...
		actual, err := TextChannelID.Parse(s, ctx)
		require.NoError(t, err)
	...
	t.Run("failure", func(t *testing.T) {
		t.Run("invalid id", func(t *testing.T) {
			...
			expect := channelIDInvalidError
			expect.Placeholders = attachDefaultPlaceholders(expect.Placeholders, ctx)

			_, actual := TextChannelID.Parse(nil, ctx)
			assert.Equal(t, errors.NewArgumentParsingErrorl(expect), actual)
		})
	})
	...
	t.Run("channel not found", func(t *testing.T) {
		...
		m.Error(http.MethodGet, "/channels/"+ctx.Raw, httputil.HTTPError{
			Status:  http.StatusNotFound,
			Code:    10003,
			Message: "Unknown channel",
		})
	...
	for _, c := range apiFailureCases {
		t.Run(c.name, func(t *testing.T) {
			...
			_, actual := TextChannelID.Parse(s, ctx)
			assert.Equal(t, errors.NewArgumentParsingErrorl(expect), actual)
